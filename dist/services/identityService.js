...
const findByEmailOrPhone = (email, phone) => {
    ...
    return database_1.default
        .prepare(`SELECT * FROM Contact WHERE (${conditions.join(" OR ")}) AND deletedAt IS NULL ORDER BY createdAt ASC`)
        .all(...params);
};
const findAllInCluster = (primaryId) => {
...
const createContact = (email, phone, linkedId, linkPrecedence) => {
    const result = database_1.default
        .prepare(`INSERT INTO Contact (email, phoneNumber, linkedId, linkPrecedence, createdAt, updatedAt)
...
const demoteToPrimary = (oldPrimaryId, newPrimaryId) => {
    // The older contact stays primary. The newer primary → secondary.
    database_1.default.prepare(`UPDATE Contact
     SET linkedId = ?, linkPrecedence = 'secondary', updatedAt = datetime('now')
     WHERE id = ?`).run(newPrimaryId, oldPrimaryId);
    // Re-link all contacts that were pointing to the old primary
    database_1.default.prepare(`UPDATE Contact
     SET linkedId = ?, updatedAt = datetime('now')
     WHERE linkedId = ? AND id != ?`).run(newPrimaryId, oldPrimaryId, oldPrimaryId);
};
// ─── Resolve root primary for a contact ──────────────────────────────────────
const getRootPrimary = (contact) => {
    if (contact.linkPrecedence === "primary")
        return contact;
    return database_1.default
        .prepare(`SELECT * FROM Contact WHERE id = ?`)
        .get(contact.linkedId);
};
// ─── Build response ───────────────────────────────────────────────────────────
const buildResponse = (primaryId) => {
